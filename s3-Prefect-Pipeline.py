...
def runner(
    bucket: str, file_path: str, runner: str, template_path: str = "path_to/ccdi_template/in/ccdi-curation/bucket", 
    sra_template_path: str = "path_to/sra_template/in/ccdi-curation/bucket", output_folder: str ="outputs"
):  
    ...
    if template_path != "path_to/ccdi_template/in/ccdi-curation/bucket":
        ...
        runner_logger.info("No CCDI template was provided and a CCDI template is downloaded from GitHub repo")
    ...
    runner_logger.info(f"The current latest version of CCDI template is v{latest_manifest_version}")
    ...
    if manifest_version == latest_manifest_version:
        ...
        if template_version != latest_manifest_version: 
            input_template = dl_ccdi_template()
            runner_logger.warning(f"CCDI Manifest was found in latest version {manifest_version}, while a version of {template_version} template was provided. The workflow continues by downloading the newest CCDI template")
        else:
            pass
    else:
        if manifest_version == template_version:
            output_folder = output_folder + "(OLD_VERSION_v" + manifest_version + ")"
            runner_logger.error(f"An old version(v{manifest_version}) of CCDI manifest and CCDI template were provided. New version of CCDI template v{latest_manifest_version} is available")
        else:
            runner_logger.error(f"An old version of CCDI manifest was provided(v{manifest_version}). And no matching CCDI template was provided. Please provide a matching CCDI template in the same version of the manifest or update the CCDI manifest to the latest version v{latest_manifest_version}")
            raise ValueError("CCDI manifest version is older version and doesn't match to the version of provided CCDI template")
    ...
    if sra_template_path != "path_to/sra_template/in/ccdi-curation/bucket":
        ...
        runner_logger.info("No SRA template was provided. A template was downloaded from GitHub repo")
    ...
    runner_logger.info(f"Uploading workflow inputs and outputs to bucket {bucket} under folder {output_folder}")
    outputs_ul(
        bucket=bucket,
