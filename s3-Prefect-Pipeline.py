...
@flow(name="Upload outputs", flow_run_name="upload_outputs_{date}")
def folder_ul(local_folder: str, bucket, destination: str, date: str) -> None:
...
@flow(
    name="S3 Prefect Pipeline test QL", log_prints=True, flow_run_name="QL_test_{date}"
)
def runner(
    bucket: str, file_path: str, template_path: str, sra_template_path: str, date: str
):  # removed profile
    # if not profile:
    profile = "default"
    instance = 0
    ...
    # validation_out_file = ValidationRy(input_file, input_template)
    ...
    # )
    (sra_out_file, sra_out_log) = CCDI_to_SRA(
        manifest=input_file, template=input_sra_template, date=get_date()
    )
    file_ul(bucket, file_path, sra_out_file)
    file_ul(bucket, file_path, sra_out_log)

    # dbgap_output_folder = CCDI_to_dbGaP(manifest=catcherr_out_file)
    (dbgap_output_folder, dbgap_out_log)= CCDI_to_dbGaP(manifest=input_file, date=get_date())
    folder_ul(
...
if __name__ == "__main__":
    ...
    file_path = "inputs/CCDI_Submission_Template_v1.7.1_30ExampleR20231201.xlsx"
    template_path = "inputs/CCDI_Submission_Template_v1.7.1.xlsx"
    sra_template_path = "inputs/phsXXXXXX.xlsx"

    runner(
        bucket, file_path, template_path, sra_template_path=sra_template_path, date=get_date()
    )  # removed profile
