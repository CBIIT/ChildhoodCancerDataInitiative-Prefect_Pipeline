...
from src.mci_monthly_release import read_mci_staing_folder, download_diff_files, find_newly_added, ProceedtoDownloadInput, MCIInputDescriptionMD
...
def mci_release_manifest(mci_manifests_bucket_path: str, bucket: str, template_tag: str, previous_template_path: str, runner: str):
    logger = get_run_logger()

    manifest_bucket, manifest_folder= parse_file_url_in_cds(url=mci_manifests_bucket_path)
    logger.info(f"The workflow will scan the bucket {manifest_bucket} folder {manifest_folder} for newly added manifests")
    ...
    # identify the diff
    diff_list, diff_filename = find_newly_added(download_list=staging_manifests_list, prev_pulled_list=prev_pull_list)
    ...
    proceed_input = pause_flow_run(
        wait_for_input=ProceedtoDownloadInput.with_initial_data(
            description=MCIInputDescriptionMD.proceed_to_download_md.format(
                today_date=get_date(),
                bucket=bucket,
                output_folder=output_folder,
    ...
    if proceed_input.proceed_to_download == "y":
        ...
        downloading_folder = download_diff_files(bucket=staging_bucket, diff_file_list=diff_list)

        # download the template of a given tag
        template_name = CCDI_Tags().download_tag_manifest(tag=template_tag, logger=logger)
        ...
        if len(manifest_files) > 0:
            ...
            logger.warning(
                f"No xlsx file found under folder {downloading_folder}"
            )

    else:
        logger.info("You decided not to proceed with manifest downloading. The workflow finished!")
