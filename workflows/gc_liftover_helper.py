...
# TASK: to load TSV files from a specified folder
...
def load_tsvs_from_folder(folder_path):
    ...
    for file in os.listdir(folder_path):
        if file.endswith(".tsv"):
            ...
            df = replace_semicolons(df, file_name)
...
# TASK: to save the dataframes to TSV files in the specified output folder
...
# HELPER: function to move the ID column to the front of the dataframe
...
# HELPER: function to replace semicolons with pipes (GC requirement) in the dataframe
def replace_semicolons(df, file_name):
    if file_name == 'diagnosis':
        saved_dx_cols = df['primary_diagnosis'].copy()
    df = df.replace(';', '|', regex=True)
    if file_name == 'diagnosis':
         df['primary_diagnosis'] = saved_dx_cols
    return df

# TASK: main task to generate GC IDs for each node
@task(
...
def generate_ids_task(sheet_dfs):
    # --- STUDY ID Generation ---
    # study_id is generated by concatenating the phs_accession and study_acronym.
    ...
    # investigator_id is generated by concatenating the study.study_id and email.
    ...
    # primary_investigator_name is parsed into title, first_name, middle_name, last_name, and suffix.
    ...
    # consent_group_number is reformatted to remove any leading 'c' characters and whitespace.
    ...
    # study_participant_id is generated by concatenating the study.study_id and participant_id.
    ...
    # study_diagnosis_id is generated by concatenating the participant.study_participant_id and diagnosis_id.
    ...
    # in the diagnosis dataframe, survival and site info are consolidated into a single row per participant.
        # load the data
        df = sheet_dfs['diagnosis'].copy()
     ...
     # treatment_id is generated by concatenating the participant.study_participant_id and therapeutic_agents.
    ...
    # The sample_type is set to 'blood' if the sample_type column contains the value 'blood', else 'analyte'
    ...
    # genomic_info_id is generated by concatenating the file.file_id and library_id (if available) with an underscore.
...
# FLOW: to orchestrate downloading, processing, and uploading TSV files
