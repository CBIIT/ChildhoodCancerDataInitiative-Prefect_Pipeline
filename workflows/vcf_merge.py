...
def merging(df: pd.DataFrame):
    ...
    dict_f_name_pid = (
        df[["File Name", "patient_id"]].set_index("File Name").to_dict()["patient_id"]
    )
    dict_f_name_sid = (
        df[["File Name", "sample_id"]].set_index("File Name").to_dict()["sample_id"]
    )
    ...
    if len(vcf_to_merge) > 1:
        ...
        for vcf in vcf_to_merge:
            ...
            vcf_df.df = vcf_df.df.rename(
                columns={
                    "tumor": dict_f_name_sid[vcf],
                    "normal": "normal" + "_" + dict_f_name_pid[vcf],
                }
            )
...
def merging_merged(merged_vcfs: list):
    ...
    if len(vcf_to_merge) > 1:
        runner_logger.info(
            f"Running merge of {len(vcf_to_merge)} previously merged VCF files...."
        )
