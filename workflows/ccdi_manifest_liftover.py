...
from src.manifest_liftover import tags_validation, liftover_tags, validate_mapping
...
def manifest_liftover(
    ...
    logger.info(f"Downlaoded mapping file {liftover_mapping_filepath} from bucket {bucket}")
    ...
    if lift_from_validation == True and lift_to_validation == True:
        ...
        raise ValueError("Version incompatibility found between manifest/template and mapping file")
    
    # validate mapping file
    validate_report = validate_mapping(manifest_path=ccdi_manifest, template_path=ccdi_template, mapping_path=mapping_file)
    logger.info(f"Created a report for mapping file evaluation: {validate_report}")
    file_ul(bucket=bucket, output_folder=output_folder, sub_folder="", newfile=validate_report)
    logger.info(f"Uploaded mapping file evaluation report to the bucket {bucket} folder {output_folder}")
