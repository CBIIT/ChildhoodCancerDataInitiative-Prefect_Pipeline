...
def concatenate_submissions(xlsx_list: list[str], template_file: str, logger) -> str:
    ...
    xlsx_list, not_matched_list = if_version_match(
        xlsx_list=xlsx_list, template_version=tempalte_version
    )
    if len(not_matched_list) != 0:
        logger.error(
            f"Found {len(not_matched_list)} submission files has version different from template:  {*not_matched_list,}"
        )
    else:
