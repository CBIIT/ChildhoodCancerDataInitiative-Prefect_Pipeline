        ...
        for id in key_record.keys():
            ...
            if len(key_record[id]) > 1:
                ...
                for j in range(len(affected_nodes)):
                    dup_key.append({"key_id_value":id, "node":affected_nodes[j]})
            else:
                pass
        # non-unique key_id found in file
        if len(dup_key) > 0:
            print_str = print_str + "Error: Found non-unique key id value in the manifest\n\t"
            ...
            print_str = print_str + "INFO: All key id values are unique across the study\n"
    else:
...
def ValidationRy_new(file_path: str, template_path: str):
    ...
    validate_unique_key_across_study(file_path=file_path, template_path=template_path, output_file=output_file)
